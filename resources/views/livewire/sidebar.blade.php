<aside class="app-sidebar shadow d-flex flex-column {{ $colorTexto }}"
    style="min-height: 100vh; background-color: {{ $colorFondo }};">
    <div class="d-flex justify-content-center py-4" style="background-color: {{ $colorFondo }};">
        <img src="{{ $logoPath }}" alt="Logo de la Empresa"
            style="height: 150px; max-width: 200px; object-fit: contain;">
    </div>
    <div class="sidebar-wrapper flex-grow-1 px-2 pt-2">
        @php
        $gestionActiva = request()->routeIs(['productos.*', 'categorias.*', 'pedidos.*']);
            ...
            <div class="nav-item mt-3">
                ...
                <div class="collapse ps-4 {{ $seguridadActiva ? 'show' : '' }}" id="adminMenu">
                    ...
                    <a href="{{ route('usuarios.index') }}"
                        class="nav-link {{ request()->routeIs(['usuarios.*']) ? 'active-item' : $colorTexto }}"><i
                            class="fa-solid fa-users me-2"></i> Usuarios</a>
                    @endcan
                    @can('rol-list')
                    <a href="{{ route('roles.index') }}"
                        class="nav-link {{ request()->routeIs(['roles.*']) ? 'active-item' : $colorTexto }}"><i
                            class="fa-solid fa-user-tag me-2"></i> Roles</a>
                    @endcan
                    @can('permission-list')
                    <a href="{{ route('permisos.index') }}"
                        class="nav-link {{ request()->routeIs('permisos.index') ? 'active-item' : $colorTexto }}"><i
                            class="fa-solid fa-key me-2"></i> Permisos</a>
                    @endcan
                </div>
            </div>
            @endcanany
            ...
            <div class="nav-item mt-3">
                ...
                <div class="collapse ps-4 {{ $gestionActiva ? 'show' : '' }}" id="gestionMenu">
                    ...
                    <a href="{{ route('categorias.index') }}"
                        class="nav-link {{ request()->routeIs(['categorias.*']) ? 'active-item' : $colorTexto }}"><i
                            class="fa-solid fa-tags me-2"></i> Categorías</a>
                    @endcan
                    @can('producto-list')
                    <a href="{{ route('productos.index') }}"
                        class="nav-link {{ request()->routeIs(['productos.*']) ? 'active-item' : $colorTexto }}"><i
                            class="fa-solid fa-box-open me-2"></i> Productos</a>
                    @endcan
                    @can('pedido-list')
                    <a href="{{ route('pedidos.index') }}"
                        class="nav-link {{ request()->routeIs('pedidos.index') ? 'active-item' : $colorTexto }}"><i
                            class="fa-solid fa-file-invoice-dollar me-2"></i> Pedidos</a>
                    @endcan
                    @if(auth()->user()->hasRole('super_admin'))
                    <a href="#" class="nav-link {{ $colorTexto }}" data-bs-toggle="modal"
                        data-bs-target="#selectEmpresaModal">
                    ...
                    <a href="{{ route('tienda.public.index', auth()->user()->empresa) }}" target="_blank"
                        class="nav-link {{ $colorTexto }}">

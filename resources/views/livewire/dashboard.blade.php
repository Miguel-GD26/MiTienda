<div>
        ...
        <div class="container-fluid">
            ...
            @elseif($view_type == 'admin')
                @if(auth()->user()->empresa)
                    ...
                    @include('livewire.dashboard.dashboard-content')
                @else
                    <div class="alert alert-warning"><strong>Atención:</strong> No tienes una empresa asignada. Contacta al Super Administrador.</div>
                @endif

            @elseif($view_type == 'repartidor')
                <div class="alert alert-info">Dashboard de Repartidor en construcción.</div>

            @elseif($view_type == 'no_empresa')
                <div class="alert alert-warning"><strong>Atención:</strong> No tienes una empresa asignada para ver el dashboard.</div>
            @else
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            Livewire.on('dashboard-rendered', () => {
                const chartEl = document.getElementById('sparkline-chart');
                if (chartEl) {
                    if (window.sparklineChart instanceof ApexCharts) window.sparklineChart.destroy();
                    const sparklineOptions = {
                      series: [{ name: 'Ingresos', data: @json($sparkline['series'] ?? []) }],
                      chart: { type: 'area', height: 160, sparkline: { enabled: true }, },
                      stroke: { curve: 'smooth', width: 2 },
                      fill: { type: 'gradient', gradient: { shade: 'light', type: "vertical", shadeIntensity: 0.3, opacityFrom: 0.7, opacityTo: 0.3, }},
                      colors: ['var(--kpi-blue)'],
                      xaxis: { type: 'datetime', categories: @json($sparkline['labels'] ?? []) },
                      tooltip: {
                    ...
                    window.sparklineChart = new ApexCharts(chartEl, sparklineOptions);
                    window.sparklineChart.render();
                }
            });
            Livewire.dispatch('dashboard-rendered');
        });
    </script>
    @endpush
</div>